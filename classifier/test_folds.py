...
def evaluate_fold(X_train, Y_train, X_test, method = "random_forest", max_depth = 8, no_trees = 500, lr = 5):
...
def main():
    ...
    #X_train_list, Y_train_list, X_test_list, Y_test_list = gen('dataset_kinect.csv')

    pred = []
    lab = []
    #md = {}

    for (X_train, Y_train, X_test, Y_test) in gen('/mnt/scratch1/siy197580/nirjhar/dataset_kinect.csv'):
        lab += Y_test.tolist()
        predictions = evaluate_fold(X_train, Y_train, X_test)#, Y_test, os.path.join(confusion_root, "fold_" + str(i) + ".png"))
        pred += predictions
    ...
    df_predictions.to_csv("predictions_cam_wise_kinect_d8.csv", index = False)
